...
if __name__ == "__main__":
    ...
    parser.add_argument("--split_perc", type=float, default=0.9)
    ...
    # CHANGE ME
    try:
        dataset = load_dataset('csv', data_files=args.training_dir+'/sig_diff_headlines.csv')
#         eval_dataset = load_dataset('csv', data_files=args.test_dir+'/eval_frame.csv')
    except:
        print('had to use alternative path')
        dataset = load_dataset('csv', data_files='/opt/ml/input/data/train/sig_diff_headlines.csv')
#         eval_dataset = load_dataset('csv', data_files='/opt/ml/input/data/test/eval_frame.csv')
...
#         token_type_ids.append(np.array(token['token_type_ids'], dtype=np.int32))
    ...
    split_ind = int(args.split_perc*len(dataset['train']))
    dataset_dict = {}
    eval_dataset_dict = {}
